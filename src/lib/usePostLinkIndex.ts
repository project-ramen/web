...
/**
 * 발행된 포스트/프로젝트의 (제목 | 슬러그) → 블로그 내부 URL 매핑을 만들어,
 * Obsidian 위키링크([[파일 이름|별칭]])의 대상이 실제 게시된 글이면 클릭 가능한 링크로
 * 해석할 수 있게 해준다. 매칭 안 되면 remarkWikilinks가 알아서 텍스트로만 표시한다.
 */
    ...
    async function load() {
      try {
        ...
        for (const p of (posts as IndexedPost[]) ?? []) {
          ...
          map.set(normalize(p.slug), href);
          if (p.title) map.set(normalize(p.title), href);
        }
